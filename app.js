...
// TEMP TEST LOGIN — REMOVE LATER
createUserWithEmailAndPassword(auth, "test@example.com", "test1234")
  .then(() => {
  ...
  .catch((error) => {
    if (error.code === "auth/email-already-in-use") {
      signInWithEmailAndPassword(auth, "test@example.com", "test1234")
        .then(() => console.log("Signed in existing user"))
...
async function loadDashboardData() {
    try {
        ...
        document.getElementById('disabledMembers').textContent = stats.disabledMembers;
        document.getElementById('seniorCitizens').textContent = stats.seniorCitizens;
        document.getElementById('students').textContent = stats.students;
        document.getElementById('pensioners').textContent = stats.pensioners;
    } catch (error) {
...
async function loadFamilies(zoneFilter = '') {
    try {
        const url = zoneFilter 
            ? `${API_BASE_URL}/families/zone/${zoneFilter}`
            : `${API_BASE_URL}/families`;
        
        const response = await fetch(url);
        familiesData = await response.json();
...
async function loadMembers(searchQuery = '') {
    try {
        const url = searchQuery 
            ? `${API_BASE_URL}/members/search?query=${encodeURIComponent(searchQuery)}`
...
async function runQuery(type) {
    try {
        ...
        switch(type) {
            case 'disabled':
                url = `${API_BASE_URL}/members/disabled`;
                break;
            case 'seniors':
                url = `${API_BASE_URL}/members/seniors`;
                break;
            case 'students':
                url = `${API_BASE_URL}/members/students`;
                break;
            case 'pensioners':
                url = `${API_BASE_URL}/members/pensioners`;
                break;
            default:
                return;
        }
        ...
        displayQueryResults(members, type);
    } catch (error) {
...
async function runCustomQuery(occupation) {
    try {
        const response = await fetch(`${API_BASE_URL}/members/occupation/${encodeURIComponent(occupation)}`);
        const members = await response.json();
        displayQueryResults(members, 'custom', occupation);
    } catch (error) {
    ...
    container.innerHTML = requests.map(request => `
        <div class="request-card">
            <div class="request-header">
                <div>
                    <div class="request-person">${request.member.name}</div>
...
async function saveFamilyForm() {
    ...
    try {
        const url = familyId 
            ? `${API_BASE_URL}/families/${familyId}`
            : `${API_BASE_URL}/families`;
...
async function saveMemberForm() {
    ...
    try {
        ...
        if (memberId) {
            ...
            url = `${API_BASE_URL}/families/${familyId}/members`;
            method = 'POST';
        }
...
async function searchPersonForRequest(query) {
    try {
        const response = await fetch(`${API_BASE_URL}/members/search?query=${encodeURIComponent(query)}`);
        const members = await response.json();
